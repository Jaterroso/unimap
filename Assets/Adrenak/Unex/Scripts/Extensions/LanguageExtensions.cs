...
namespace Adrenak.Unex {
    public static class LanguageExtensions {
		...
		public static void TryInvoke<T, K>(this Action<T, K> action, T t, K k) {
			if (action != null)
				action(t, k);
		}
		...
		public static string GetPathExtension(this string path) {
			var splits = GetPathFile(path).Split('.');
			if (splits.IsNullOrEmpty()) return string.Empty;
			return splits[splits.Length - 1].Split('?')[0];
		}

		public static string GetPathFile(this string path) {
			var splits = path.Split(new[] { '\\', '/' });
			if (splits.IsNullOrEmpty()) return string.Empty;
			return splits[splits.Length - 1].Split('?')[0];
		}

		public static bool ContainsAny(this string first, string[] check, out int index) {
